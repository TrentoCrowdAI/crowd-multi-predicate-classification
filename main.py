...
from helpers.utils import run_quiz_criteria_confm, get_roc_points, get_auc
...
if __name__ == '__main__':
    ...
    for Nt in [4]:
        for J in [5]:
            ...
            auc_df = pd.DataFrame({'auc_b': [auc_b], 'auc_m': [auc_m], 'auc_sm': [auc_sm]})
            data = pd.concat([auc_df, roc_b_df, roc_m_df, roc_sm_df], ignore_index=True)
            data.to_csv('output/data/roc_curves43.csv', index=False)
