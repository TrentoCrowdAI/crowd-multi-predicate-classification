            ...
            for acc_term in [10, -10, 'gt', 'est']:
                for theta in criteria_power.keys():
                    for M in [3, 5, 10, 100]:
                        ...
                        print 'SM-RUN  M: {}, acc_term: {}, theta: {}, loss: {:1.2f}, price: {:1.2f}, fp_rate: {:1.2f}, tp_rate: {:1.2f}, ' \
                              'recall: {:1.2f}, precision: {:1.2f}, f_b: {}'. \
                            format(M, str(acc_term), theta, np.mean(loss_smrun_list), np.mean(cost_smrun_list), np.mean(fp_sm), np.mean(tp_sm),
                        ...
                        data.append([Nt, J, lr, np.mean(loss_smrun_list), np.std(loss_smrun_list), np.mean(fp_sm),
                                     ...
                                     np.mean(rec_sm), np.mean(pre_sm), np.mean(f_sm), str(acc_term), theta, M])
                    print '---------------------'
    pd.DataFrame(data, columns=['Nt', 'J', 'lr', 'loss_mean', 'loss_std', 'FPR', 'TPR',
                                'price_mean', 'price_std', 'alg', 'recall', 'precision', 'f_beta', 'acc_term', 'theta', 'M']). \
                                to_csv('output/data/accurcy_criteria_experiment_varyM.csv', index=False)
