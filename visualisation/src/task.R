...
# accuracy of results
library(ggplot2)
data <- task_z03_tests1_papers100
...
ggplot(acc_plot, aes(x=papers_page, y=budget_spent_avg, group=judgment, col=judgment, fill=judgment)) + 
  geom_errorbar(aes(ymin=budget_spent_avg-budget_spent_std, ymax=budget_spent_avg+budget_spent_std), width=.1, position=pd) +
  geom_line(position=pd) +
  geom_point(position=pd) +
  scale_x_continuous(breaks=c(1,2,3,4,5,6,7,8,9)) +
...
data <- task_results_cost
ggplot(data, aes(x=papers_page, y=(fn_mv_lose_avg+fp_mv_lose_avg))) + 
  geom_errorbar(aes(ymin=(fn_mv_lose_avg+fp_mv_lose_avg)-(fp_mv_lose_std+fn_mv_lose_std)/2, ymax=(fn_mv_lose_avg+fp_mv_lose_avg)+(fp_mv_lose_std+fn_mv_lose_std)/2), width=.1, position=pd) +
  geom_line(position=pd) +
  geom_point(position=pd) +
  scale_x_continuous(breaks=c(1,2,3,4,5,6,7,8,9)) +
  ggtitle("Total MV lose VS papers per page") +
  xlab('Papers per page') +
  ylab('Total lose')
