...
if __name__ == '__main__':
    ...
    for Nt in [3, 5]:
        for J in [3, 5]:
            for acc_term in [0.5, -0.5, 0.1, -0.1, 'random', None]:
                ...
                print 'SM-RUN  acc_term: {}, loss: {:1.2f}, price: {:1.2f}, fp_rate: {:1.2f}, tp_rate: {:1.2f}, ' \
                      'recall: {:1.2f}, precision: {:1.2f}, f_b: {}'.\
                    format(str(acc_term), np.mean(loss_smrun_list), np.mean(cost_smrun_list), np.mean(fp_sm), np.mean(tp_sm),
                ...
                data.append([Nt, J, lr, np.mean(loss_smrun_list), np.std(loss_smrun_list), np.mean(fp_sm),
                             ...
                             np.mean(rec_sm), np.mean(pre_sm), np.mean(f_sm), str(acc_term)])
            print '---------------------'
    pd.DataFrame(data, columns=['Nt', 'J', 'lr', 'loss_mean', 'loss_std', 'FPR', 'TPR',
                                'price_mean', 'price_std', 'alg', 'recall', 'precision', 'f_beta', 'acc_term']). \
